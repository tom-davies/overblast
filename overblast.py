...
def blast(sequence, run):
    "Send Sequence to BLASTn"
    print("Running BLAST , Please Wait.")
    result_handle = NCBIWWW.qblast("blastn", "nr", sequence, entrez_query="all[filter] NOT((srcdb_refseq_model[prop] AND biomol_rna[prop]))")
    blast_file = open("blast%d.xml","w" %run)
    blast_file.write(result_handle.read())
    blast_file.close()
    result_handle.close()
    return;

#blast(sequence = seq_origin.seq,1)

def tax(run):
    result_handle = open("blast%d.xml" %run)
